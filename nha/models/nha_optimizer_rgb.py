...
class TextureModule(torch.nn.Module):
    def forward(self, pixel_uv_coords, pixel_uv_ids, **kwargs):
        """
        Get RGB from passed uv coordinates and uv ids

        Input:
            pixel_uv_coords: (B, N, 2)
            pixel_iv_ids: (B, N, 2)
...
class RGBTexture(TextureModule):
    def __init__(self, face_res, teeth_res):
        super().__init__()
        self._explFeatures = MultiTexture(
            torch.randn(3, face_res, face_res) * 0.01,
            torch.randn(3, teeth_res, teeth_res) * 0.01,
        )

    def forward(self, pixel_uv_coords, pixel_uv_ids):
        """
        Sample colors in the trained texture
        ...
        Returns:
            pixel_colors: (N, 3)
        """
        rgb_colors = self._explFeatures(
            pixel_uv_coords[:, None, None], 
            pixel_uv_ids[:, None, None]
        )[:, :, 0, 0]
        return torch.tanh(rgb_colors)



class NHAOptimizerRGB(pl.LightningModule):
    """
    ...
    def _render_rgba(self, verts, K, RT, H, W, expr, pose, mouth_cond, return_rasterizer_results=False,
        ...
        mlp_pixel_rgb[mask] = self._texture(pixel_uv_coords_masked, pixel_uv_ids_masked)
