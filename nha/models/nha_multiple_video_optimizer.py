from nha.models.texture3d import MultiSubjectMultiTexture
...
class NHAMultipleVideoOptimizer(pl.LightningModule):
    """
    ...
    def __init__(self, num_videos, frames_per_video, w_lap, w_silh, w_semantic_hair, body_part_weights, **kwargs):
        super().__init__()
        ...
        self._jaw_pose = torch.nn.Parameter(torch.zeros(self._num_frames, 3), requires_grad=True)
        self._eyes_pose = torch.nn.Parameter(torch.zeros(self._num_frames, 6), requires_grad=True)
        self._translation = torch.nn.Parameter(torch.zeros(self._num_frames, 3), requires_grad=True)
        self._rotation = torch.nn.Parameter(torch.zeros(self._num_frames, 3), requires_grad=True)
        ...
        self._vert_feats = torch.nn.Parameter(torch.zeros(self._num_videos, len(self._offset_indices), 32), requires_grad=True)
        ...
        self._explFeatures = MultiSubjectMultiTexture(
            torch.randn(num_videos, d_feat, face_res, face_res) * 0.01,
            torch.randn(num_videos, d_feat, teeth_res, teeth_res) * 0.01,
        )
    ...
    def _predict_offsets(self, neck_rot, video_id):
        """
        ...
        :param video_id: tensor of video ids N
    ...
    def _get_frame_id(self, video_id, frame_num):
        return self._frame_count_prefixes[video_id] + frame_num
    ...
    def _create_flame_param_batch(self, batch, ignore_shape=False, ignore_expr=False, ignore_pose=False,
        ...
        frame_num = batch['frame']
        frame_id = self._get_frame_id(video_id, frame_num)
        ...
        if ignore_shape:
            ...
            p["shape"] = self._shape[video_id]

        if ignore_expr:
            p["expr"] = 0
        else:
            p["expr"] = self._expr[frame_id]
        ...
        if ignore_pose:
            ...
            p["neck"] = self._neck_pose[frame_id]  # * torch.tensor([[0, 1., 0]], device=self.device)
            p["jaw"] = self._jaw_pose[frame_id]
            p["eyes"] = self._eyes_pose[frame_id]
            p["rotation"] = self._rotation[frame_id]
            p["translation"] = self._translation[frame_id]
    ...
    def _render_rgba(self, verts, K, RT, H, W, expr, pose, mouth_cond, video_id, return_rasterizer_results=False,
    ...
    def forward(self, batch, ignore_expr=False, ignore_pose=False, center_prediction=False, symmetric_rgb_range=True):
        ...
        rgba_pred = self._render_rgba(offsets_verts, K, RT, H, W, expr=expr, pose=pose, mouth_cond=mouth_conditioning, video_id=batch["video_id"],
                                      center_prediction=center_prediction)
