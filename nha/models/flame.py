...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "_v_template", to_tensor(to_np(flame_model.v_template), dtype=self.dtype)
        )
        ...
        shapedirs = torch.cat(
            [shapedirs[:, :, :shape_params], shapedirs[:, :, 300 : 300 + expr_params]], 2
        )
        ...
        self.register_buffer(
            "dynamic_lmk_bary_coords", lmk_embeddings["dynamic_lmk_bary_coords"].float()
        )
        self.register_buffer(
            "full_lmk_faces_idx",
        ...
        faces_coords_normed /= (
            faces_coords_normed.flatten(end_dim=1).max(dim=0).values.view(1, 1, 3)
        )
        ...
        vertex_uvmap[
            torch.cat([self._parts.get("mouth", zero_index), self._parts.get("teeth", zero_index)])
        ] = 1
        ...
        if len(self._ignore_faces) != 0:
            ...
            self.register_buffer(
                "_v_template_normed_filtered",
                self._v_template_normed[self._vert_filter],
                persistent=False,
            )
            self.spatially_blurred_vert_labels = self.spatially_blurred_vert_labels[
                filter_dict["vert_filter"]
            ]
        ...
        self.register_buffer(
            "vert_labels",
            torch.zeros(len(self.v_template), len(self.get_body_parts()), dtype=torch.float),
            persistent=False,
        )
        for i, (key, verts) in enumerate(self.get_body_parts().items()):
            self.vert_labels[verts, i] = 1.0
    ...
    def v_template_normed(self):
        return (
            self._v_template_normed_filtered
            if len(self._ignore_faces) != 0
    ...
    def face_coords(self):
        return (
            self._face_coords[self._face_filter]
            if len(self._ignore_faces) != 0
    ...
    def face_coords_normed(self):
        return (
            self._face_coords_normed[self._face_filter]
            if len(self._ignore_faces) != 0
            else self._face_coords_normed
        )
    ...
    def face_uvcoords(self):
        return (
            self._face_uvcoords[self._face_filter]
            if len(self._ignore_faces) != 0
            else self._face_uvcoords
        )

    @property
    def face_uvmap(self):
        return (
            self._face_uvmap[self._face_filter]
            if len(self._ignore_faces) != 0
            else self._face_uvmap
        )

    @property
    def face_normals(self):
        return (
            self._face_normals[self._face_filter]
            if len(self._ignore_faces) != 0
    ...
    def vertex_uvs(self):
        return (
            self._vertex_uvs[self._uv_filter] if len(self._ignore_faces) != 0 else self._vertex_uvs
        )

    @property
    def vertex_normals(self):
        return (
            self._vertex_normals[self._vert_filter]
            if len(self._ignore_faces) != 0
            else self._vertex_normals
        )
    ...
    def _get_spatially_blurred_vert_labels(self, sigma=0.01):
        ...
        distances[e1, e2] = distances[e2, e1] = torch.linalg.norm(
            verts[e1] - verts[e2], dim=-1, ord=2
        )
        max_neighbours = (
            torch.mean(torch.sum(distances < UPPER_PLACEHOLDER, dim=-1).float() - 1) * 2
        ).int()
        assert (
            torch.max(distances) == UPPER_PLACEHOLDER
        )  # check if upper placeholder was set high enough
        ...
        for i in range(5):
            ...
            distances = torch.min(
                distances[neighbours] + distances[v_helper, neighbours].unsqueeze(-1), dim=1
            ).values

        weights = torch.softmax((-(distances**2) / (2 * sigma**2)), dim=-1)

        vert_labels = torch.zeros(
            V, len(self._parts), dtype=torch.float, device=self._v_template.device
        )
        for i, (key, verts) in enumerate(self._parts.items()):
            vert_labels[verts, i] = 1.0
        ...
        for key, val in upsample_regions.items():
            if key != "all" and val > 0:
                raise NotImplementedError(
                    "Seperated upsampling of different regions not implemented."
                )

        if "all" in upsample_regions and upsample_regions["all"] > 0:
            for i in range(upsample_regions["all"]):
                eye_vert_idcs = self.get_body_part_vert_idcs(
                    *[part for part in self._parts if "eyeball" in part],
                    consider_full_flame_model=True,
                )
                eye_face_idcs = self.faces_of_verts(
                    eye_vert_idcs, consider_full_flame_model=True, return_face_idcs=True
                )[1]
                face_idcs = list(
                    set(range(len(self._faces))) - set(self._ignore_faces) - set(eye_face_idcs)
                )
    ...
    def _upsample_faces(self, face_idcs):
        ...
        faces = torch.cat(
            (
                self._faces,
                torch.tensor(
                    faces[len(face_idcs) :], dtype=self._faces.dtype, device=self._faces.device
                ),
            ),
            dim=0,
        )
        uv_faces = torch.cat(
            (
                self._faces_uvs,
                torch.tensor(
                    uv_faces[len(face_idcs) :],
                    dtype=self._faces_uvs.dtype,
                    device=self._faces_uvs.device,
                ),
            ),
            dim=0,
        )
        ...
        self.register_buffer(
            "_v_template", torch.tensor(verts, dtype=self._v_template.dtype), persistent=False
        )
        self.register_buffer(
            "_vertex_uvs", torch.tensor(uvs, dtype=self._vertex_uvs.dtype), persistent=False
        )
        ...
        self.register_buffer(
            "shapedirs", torch.cat((self.shapedirs, new_shapedirs), dim=0), persistent=False
        )
        self.register_buffer(
            "posedirs", torch.cat((self.posedirs, new_posedirs), dim=1), persistent=False
        )

        # calculate new lbs components
        new_J_regressor = (
            torch.zeros(5, n_edges).to(self.J_regressor.dtype).to(self.J_regressor.device)
        )
        new_lbs_weights = self.lbs_weights[edges]  # n_edges x 2 x 5
        new_lbs_weights = new_lbs_weights.mean(dim=1)  # n_edges x 5
        self.register_buffer(
            "J_regressor", torch.cat((self.J_regressor, new_J_regressor), dim=1), persistent=False
        )
        self.register_buffer(
            "lbs_weights", torch.cat((self.lbs_weights, new_lbs_weights), dim=0), persistent=False
        )
        ...
        if ignored_upsampled_faces:
            ...
            ignored_upsampled_faces_idcs = np.array(
                [face_idcs.index(f) for f in ignored_upsampled_faces]
            )
        ...
        # ignore old faces
        self._ignore_faces = list(
            set.union(set(self._ignore_faces), set(face_idcs), set(new_ignored_faces))
        )

        # calculate blurred vert labels
        new_spatially_blurred_vert_labels = self.spatially_blurred_vert_labels[edges].mean(dim=1)
        self.spatially_blurred_vert_labels = torch.cat(
            (self.spatially_blurred_vert_labels, new_spatially_blurred_vert_labels), dim=0
        )
    ...
    def save_2_obj(
        ...
        if shape is None:
            shape = torch.zeros(
                1, 300, device=self._v_template.device, dtype=self._v_template.dtype
            )
        if expr is None:
            expr = torch.zeros(1, 100, device=self._v_template.device, dtype=self._v_template.dtype)
        if rotation is None:
            rotation = torch.zeros(
                1, 3, device=self._v_template.device, dtype=self._v_template.dtype
            )
        if neck is None:
    ...
    def get_body_parts(self, consider_full_flame_model=False):
        ...
        """
        return (
            self._parts_filtered
            if len(self._ignore_faces) != 0 and not consider_full_flame_model
            else self._parts
        )
    ...
    def get_neutral_joint_rotations(self):
        ...
        for name, limit in zip(
            ["neck", "jaw", "global", "eyes"],
            [self.neck_limits, self.jaw_limits, self.global_limits, self.eye_limits],
        ):
    ...
    def _compute_surface_area(self, vertices, faces):
        ...
        face_cross_results = torch.cross(
    ...
    def _get_neighboring_face_pairs(self, faces):
        ...
        for i in range(3):
            edges_result[
                faces_e_remapped[:, i],
                torch.minimum(edges_saved[faces_e_remapped[:, i]], torch.tensor(2)),
            ] = faces_indices
    ...
    def get_curvature(self, verts):
        ...
        face_normals = face_cross_results / torch.linalg.norm(
            face_cross_results, dim=-1, keepdim=True
        )  # (N, F, 3)
        neighboring_normals = face_normals[:, self.face_neighbor_pairs]  # (N, E, 2, 3)
        neighboring_dots = (neighboring_normals[:, :, 0] * neighboring_normals[:, :, 1]).sum(
            -1
        )  # (N, E)
        ...
        **kwargs,
        ...
        if use_rotation_limits:
            ...
            if eyes is None:
                ...
                eyes = torch.cat(
                    [
                        self._apply_rotation_limit(eyes[:, :3], self.eye_limits),
                        self._apply_rotation_limit(eyes[:, 3:], self.eye_limits),
                    ],
                    dim=1,
                )
        ...
        if return_landmarks is not None:
            ...
            if return_landmarks == "static":
                ...
                landmarks = vertices2landmarks(
                    vertices, self._faces, lmk_faces_idx, lmk_bary_coords
                )
        ...
        if return_mouth_conditioning:
            mouth_vert_pair_idcs = torch.tensor(
                [
                    [3506, 3531],
                    [1670, 2906],
                    [1738, 2882],
            ...
            mouth_conditioning = (
                (mouth_vert_coords[:, 0] - mouth_vert_coords[:, 1])
                .pow(2)
                .sum(dim=-1)
                .sqrt()
                .permute(1, 0)
            ...
            mins = torch.tensor(
                [[0.0072, 0.0238, 0.0338, 0.0409, 0.0344, 0.0221, 0.0037, 0.0070, 0.0069, 0.0033]],
                device=mouth_conditioning.device,
            )
            maxs = torch.tensor(
                [[0.0237, 0.0368, 0.0500, 0.0631, 0.0508, 0.0358, 0.0157, 0.0239, 0.0229, 0.0157]],
                device=mouth_conditioning.device,
            )
